...
{
	private static String TAG="AnimatedTabHostListener";
	...
	class MyGestureDetector extends SimpleOnGestureListener
	{
		...
		{
			maxTabs = 3; //tabHost.getTabContentView().getChildCount();
		}
...
Log.d(TAG,"MotionCaptured");			
			int newTab = 0;
			if (Math.abs(event1.getY() - event2.getY()) > SWIPE_MAX_OFF_PATH){
Log.d(TAG,"SWIPE_MAX_OFF_PATH check failed");
				return false;
			}
			if (event1.getX() - event2.getX() > SWIPE_MIN_DISTANCE && Math.abs(velocityX) > SWIPE_THRESHOLD_VELOCITY){
Log.d(TAG,"1-2 SWIPE_MIN_DISTANCE ok");				
				// Swipe right to left
				newTab = currentTab + 1;
			}
			else if (event2.getX() - event1.getX() > SWIPE_MIN_DISTANCE
					&& Math.abs(velocityX) > SWIPE_THRESHOLD_VELOCITY)	{
Log.d(TAG,"2-1 SWIPE_MIN_DISTANCE ok");						
				// Swipe left to right
				newTab = currentTab - 1;
			}
